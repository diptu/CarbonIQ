...
PASSWORD="Hello123"
GATEWAY_AUTH_URL="http://localhost:9003"
...
echo
echo "=== REFRESH TOKEN ==="
REFRESH_RESPONSE=$(curl -s -X POST "$GATEWAY_AUTH_URL/auth/refresh" \
  -H "Content-Type: application/json" \
...
echo
echo "=== LOGOUT ==="
LOGOUT_RESPONSE=$(curl -s -X POST "$GATEWAY_AUTH_URL/auth/logout" \
  -H "Content-Type: application/json" \
...
POST_LOGOUT_REFRESH=$(curl -s -X POST "$GATEWAY_AUTH_URL/auth/refresh" \
  -H "Content-Type: application/json" \
  -d "{\"refresh_token\": \"$NEW_REFRESH_TOKEN\"}")
